    ...
    def parse_actions(self):
        ...
        def gotPage(x):
            ...
            for var_node in tree.findall('.//{%s}stateVariable' % ns):
                ...
                instance = 0
                self._variables.get(instance)[name] = variable.StateVariable(self, name, 'n/a', instance, send_events, data_type, values)
...
class ServiceServer(log.Loggable):
    ...
    def __init__(self, p_id, version, backend):
        self.id = p_id
        ...
        self.service_type = 'urn:%s:service:%s:%d' % (self.namespace, p_id, int(self.version))
    ...
    def create_new_instance(self, instance):
        ...
        for v in self._variables[0].values():
            self._variables[instance][v.name] = variable.StateVariable(v.service, v.name, v.implementation, instance, v.send_events, v.data_type, v.allowed_values)
    ...
    def build_single_notification(self, instance, variable_name, value):
        ...
        root.attrib['xmlns:e'] = 'urn:schemas-upnp-org:event-1-0'
        e = ET.SubElement(root, 'e:property')
        _s = ET.SubElement(e, variable_name).text = str(value)
    ...
    def register_vendor_variable(self, name, implementation = 'optional',
        ...
        if evented == 'never':
            ...
            send_events = evented
        new_variable = variable.StateVariable(self, name, implementation, instance, send_events, data_type, allowed_values)
    ...
    def init_var_and_actions(self):
        ...
        for action_node in tree.findall('.//action'):
            ...
            if action_node.attrib.get('{urn:schemas-beebits-net:service-1-0}X_needs_backend', None) != None:
                needs_callback = True
            if action_node.find('Optional') != None:
                implementation = 'optional'
                if (action_node.find('Optional').attrib.get('{urn:schemas-beebits-net:service-1-0}X_needs_backend', None) != None or
                    action_node.attrib.get('{urn:schemas-beebits-net:service-1-0}X_needs_backend', None) != None):
                    needs_callback = True
            arguments = []
        ...
        for var_node in tree.findall('.//stateVariable'):
            ...
            for allowed in var_node.findall('.//allowedValue'):
                values.append(allowed.text)
            self._variables.get(instance)[name] = variable.StateVariable(self, name, implementation, instance, send_events, data_type, values)
            ...
            if var_node.find('sendEventsAttribute') != None:
                never_evented = var_node.find('sendEventsAttribute').attrib.get('{urn:schemas-beebits-net:service-1-0}X_no_means_never', None)
            ...
            if allowed_value_list != None:
                vendor_values = allowed_value_list.attrib.get('{urn:schemas-beebits-net:service-1-0}X_withVendorDefines', None)
                if service_value_defaults:
                    variable_value_defaults = service_value_defaults.get(name, None)
                    if variable_value_defaults:
                        self.info("overwriting %s default value with %s" % (name, variable_value_defaults))
                    ...
                    if(vendor_values != None):
                        if service_range_defaults:
                            ...
                            if(variable_range_defaults != None and
                                ...
                                self.info("overwriting %s attribute %s with %s" % (name, e.tag, str(variable_range_defaults[e.tag])))
                                l_range[e.tag] = variable_range_defaults[e.tag]
                            elif e.text == None:
                                self.info("missing vendor definition for %s, attribute %s" % (name, e.tag))
                self._variables.get(instance)[name].set_allowed_value_range(**l_range)
                if vendor_values != None:
                    self._variables.get(instance)[name].has_vendor_values = True
...
class ServiceControl:
    ...
    def soap__generic(self, *args, **kwargs):
        ...
        for arg_name, _arg in kwargs.iteritems():
        ...
        if len(in_arguments) > 0:
            self.critical('argument %s missing for action %s' % ([ a.get_name() for a in in_arguments], action.name))
            return failure.Failure(errorCode(402))
