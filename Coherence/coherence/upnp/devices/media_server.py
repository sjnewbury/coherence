""" A device class to handle MediaRenderer.
...
class MSRoot(resource.Resource, log.Loggable):
    ...
    def getChildWithDefault(self, path, request):
                ...
                def got_item(ch):
                    if ch is not None:
                        ...
                        if os.path.exists(L_file):
                            self.info("got cover %s" % L_file)
                            return StaticFile(L_file)
                    request.setResponseCode(404)
                    ...
                    try:
                        ...
                        if 'transcoded' in request.args:
                            if self.server.coherence.config.get('transcoding', 'no') == 'yes':
                                l_format = request.args['transcoded'][0]
                                l_type = request.args['type'][0]
                                self.info("request transcoding %r %r" % (l_format, l_type))
                                try:
                                    from coherence.transcoder import TranscoderManager
                                    manager = TranscoderManager(self.server.coherence)
                                    return manager.select(l_format, ch.item.attachments[request.args['attachment'][0]])
                                except:
        ...
        #    if self.server.coherence.config.get('transcoding', 'no') == 'yes':
        #        l_id = path[:-15].split('/')[-1]
        #        self.info("request transcoding to %r for id %s" % (request.args,l_id))
        #        ch = self.store.get_by_id(l_id)
        #        uri = ch.get_path()
           ...
           TRANSCODED_REQUEST_INDICATOR.match(request.uri)):
            ...
            if self.server.coherence.config.get('transcoding', 'no') == 'yes':
                def got_stuff_to_transcode(ch):
                    ...
                    l_format = request.uri.split('/')[-1] #request.args['transcoded'][0]
                    uri = ch.get_path()
                    try:
                        from coherence.transcoder import TranscoderManager
                        manager = TranscoderManager(self.server.coherence)
                        return manager.select(l_format, uri)
                    except:
                        self.debug(traceback.format_exc())
                        request.setResponseCode(404)
                        return static.Data('<html><p>the requested transcoded file was not found</p></html>', 'text/html')
    ...
    def requestFinished(self, result, l_id, request):
        self.info("finished, remove %d from connection table" % l_id)
        ...
        self.server.connection_manager_server.remove_connection(l_id)
    ...
    def list_content(self, name, item, request):
        ...
        if(hasattr(item, 'mimetype') and item.mimetype in ['directory', 'root']):
            ...
            def build_page(r, page):
                ...
                if r is not None:
                    for c in r:
                        ...
                        page += '<li><a href="%s">%s</a></li>' % (path, title)
                page += """</ul>"""
                page += """</body></html>"""
            ...
            page += """<p><img src="%s" alt="%s"></p>""" % (path, title)
        else:
            pass
        page += """</body></html>"""
        return static.Data(page, 'text/html')
    ...
    def import_response(self, result, p_id):
        return static.Data('<html><p>import of %s finished</p></html>' % p_id, 'text/html')
    ...
    def __init__(self, hostname, uuid, urlbase,
        ...
        if len(services):
            ...
            for service in services:
                l_id = service.get_id()
                if xbox_hack == False and l_id == 'X_MS_MediaReceiverRegistrar':
                    continue
                ...
                #ET.SubElement(s, 'SCPDURL').text = l_id + '/' + service.scpd_url
                #ET.SubElement(s, 'controlURL').text = l_id + '/' + service.control_url
                #ET.SubElement(s, 'eventSubURL').text = l_id + '/' + service.subscription_url
        if len(devices):
