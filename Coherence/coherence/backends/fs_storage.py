...
Simply serves all media files from a directory and its subdirectories.
...
class FSItem(BackendItem):
    ...
    def __init__(self, object_id, parent, path, mimetype, urlbase, UPnPClass, update = False, store = None):
        ...
        @param UPnPClass:
        @param update:
        @param store: a dict of FSStore items
        """
        self.id = object_id
        self.parent = parent
        if parent:
        ...
        if mimetype in ['directory', 'root']:
            ...
            if host_port.find(':') != -1:
                host, _port = tuple(host_port.split(':'))
            else:
                host = host_port
            try:
    ...
    def set_path(self, path = None, extension = None):
        ...
        if extension is not None:
            path, _old_ext = os.path.splitext(path)
...
class FSStore(BackendStore):
    ...
    def __init__(self, server, **kwargs):
        ...
        Supported KW Args:
          urlbase = ???
          name = Name to use for the store (friendly name?)
          content = the directroy to load
          enable_notify - if 'yes' and inotify is installed - send notifications for ???
          enable_destroy - if 'yes' - ???
          import_folder -
          create_root - True/False
        """
        ...
        if(len(self.content) > 1 or
           ...
           self.import_folder != None):
            ...
            l_id = str(self.getnextID())
            parent = self.store[l_id] = FSItem(l_id, parent, 'media', 'root', self.urlbase, UPnPClass, update = True, store = self)
        if self.import_folder != None:
            l_id = str(self.getnextID())
            self.store[l_id] = FSItem(l_id, parent, self.import_folder, 'directory', self.urlbase, UPnPClass, update = True, store = self)
            self.import_folder_id = l_id
    ...
    def get_by_id(self, p_id):
        #print "get_by_id", p_id, type(p_id)
        # we have referenced ids here when we are in WMC mapping mode
        if isinstance(p_id, basestring):
            p_id = p_id.split('@', 1)
            p_id = p_id[0]
        if p_id == '0':
            p_id = '1000'
        #print "get_by_id 2", p_id
        try:
            r = self.store[p_id]
        except:
            r = None
    ...
    def get_url_by_name(self, parent = '0', name = ''):
        ...
        l_id = self.get_id_by_name(parent, name)
        #print 'get_url_by_name', l_id
        if l_id == None:
            return ''
        return self.store[l_id].url
    ...
    def remove_content_folder(self, path):
        ...
        if path in self.content:
            l_id = self.get_id_by_name('1000', path)
            self.remove(l_id)
    ...
    def create(self, mimetype, path, parent):
        ...
        if mimetype in ('root', 'directory'):
            ...
            l_id = str(l_id) + ext.lower()
        update = False
        if hasattr(self, 'update_id'):
            update = True

        self.store[l_id] = FSItem(l_id, parent, path, mimetype, self.urlbase, UPnPClass, update, store = self)
        if hasattr(self, 'update_id'):
            self.update_id += 1
    ...
    def append(self, path, parent):
        ...
        try:
            ...
            l_id = self.create(mimetype, path, parent)

            if mimetype == 'directory':
                if self.inotify is not None:
                    mask = IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO | IN_CHANGED
                    self.inotify.watch(path, mask = mask, auto_add = False, callbacks = (self.notify, l_id))
    ...
    def remove(self, p_id):
        print 'FSSTore remove id', p_id
        try:
            item = self.store[p_id]
    ...
    def notify(self, iwp, filename, mask, parameter = None):
        ...
        if(mask & IN_DELETE or mask & IN_MOVED_FROM):
            ...
            l_id = self.get_id_by_name(parameter, os.path.join(iwp.path, filename))
            if l_id != None:
                self.remove(l_id)
    ...
    def upnp_ImportResource(self, *args, **kwargs):
        ...
        if DestinationURI.endswith('?import'):
            l_id = DestinationURI.split('/')[-1]
            l_id = l_id[:-7] # remove the ?import
        else:
            return failure.Failure(errorCode(718))
        item = self.get_by_id(l_id)
        if item == None:
            return failure.Failure(errorCode(718))
    ...
    def upnp_CreateObject(self, *args, **kwargs):
        ...
        if item.upnp_class == 'object.container.storageFolder':
            ...
            l_id = self.create('directory', path, parent_item)
            try:
                os.mkdir(path)
            except:
                self.remove(l_id)
            ...
            if self.inotify is not None:
                mask = IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO | IN_CHANGED
                self.inotify.watch(path, mask = mask, auto_add = False, callbacks = (self.notify, l_id))
            new_item = self.get_by_id(l_id)
            didl = DIDLElement()
            didl.addItem(new_item.item)
            return {'ObjectID': l_id, 'Result': didl.toString()}
        if item.upnp_class.startswith('object.item'):
            ...
            l_id = self.create('item', path, parent_item)
            new_item = self.get_by_id(l_id)
