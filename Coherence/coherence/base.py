...
from coherence.upnp.core.utils import get_ip_address, get_host_address # , parse_xml
...
class Coherence(log.Loggable):
    ...
    def _setup_network_if(self):
        ...
        if self.network_if:
            self.hostname = get_ip_address('%s' % self.network_if)
        else:
            try:
    ...
    def _setup_logging(self):
        """ Initializes the logsystem

        A COHERENCE_DEBUG environment variable overwrites all level settings here.
        """
        try:
            logmode = self.config.get('logging').get('level', 'warning')
        except (KeyError, AttributeError):
            logmode = self.config.get('logmode', 'warning')
        _debug = []
        try:
            ...
            if isinstance(plugins, dict):
                ...
                for plugin in plugins:
                    try:
                        ...
                        pass
                    self.installed_plugins.append(plugin['name'])
                    try:
                        ...
                        self.warning("Can't enable plugin, %s: %s!" % (plugin, msg))
                        self.info(traceback.format_exc())
        #print "found {0:} plugins".format(self.installed_plugins)
        pass

    def _setup_control_point(self):
    ...
    def _setup_dbus(self):
        ...
        if self.config.get('use_dbus', 'no') == 'yes':
            try:
                ...
                self._setup_mirabeau()

    def _setup_mirabeau(self):
    ...
    def setup_part2(self):
        ...
        self.unittest = self._setup_unittest()
        self.ssdp_server = SSDPServer(test = self.unittest, interface = self.hostname)
        ...
        self.msearch = MSearch(self.ssdp_server, test = self.unittest)
        ...
        self._setup_control_point()
        self._setup_json()
        self._setup_transcoding()
        self._setup_dbus()

    def add_plugin(self, plugin, **kwargs):
        #print "   Adding plugin ", plugin, kwargs
