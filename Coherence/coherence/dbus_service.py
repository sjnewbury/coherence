            ...
            if hasattr(connection, '_tube') == True:
                if name == 'Browse':
                    ...
                    for item in didl.getItems():
                        ...
                        for res in item.res:
                            remote_protocol, remote_network, _remote_content_format, _ = res.protocolInfo.split(':')
...
class DBusPontoon(dbus.service.Object, log.Loggable):
    ...
    def __init__(self, controlpoint, bus = None):
        ...
        dbus.service.Object.__init__(self, self.bus, bus_name = self.bus_name, object_path = OBJECT_PATH)
    ...
    def create_oob(self, p_file):
        print 'create_oob'
        key = str(time.time())
        self.pinboard[key] = p_file
        print self.pinboard
    ...
    @dbus.service.method(BUS_NAME, in_signature = 'i', out_signature = 'av', async_callbacks = ('dbus_async_cb', 'dbus_async_err_cb'))
    ...
    @dbus.service.method(BUS_NAME, in_signature = 'ssa{ss}', out_signature = 'v', async_callbacks = ('dbus_async_cb', 'dbus_async_err_cb'))
    ...
    @dbus.service.method(BUS_NAME, in_signature = 'sss', out_signature = 'v', async_callbacks = ('dbus_async_cb', 'dbus_async_err_cb'))
    ...
    def _device_detected(self, device):
        l_id = device.get_id()
        #print "new_device_detected",device.get_usn(),device.friendly_device_type, l_id
        if l_id not in self.devices:
            new_device = DBusDevice(device, self.bus)
            self.devices[l_id] = new_device
            #print self.devices, l_id
            info = new_device.get_info()
            self.device_detected(info, l_id)
            if device.get_friendly_device_type() == 'MediaServer':
                self.UPnP_ControlPoint_MediaServer_detected(info, l_id)
            elif device.get_friendly_device_type() == 'MediaRenderer':
                self.UPnP_ControlPoint_MediaRenderer_detected(info, l_id)

    def _device_removed(self, usn = ''):
        #print "_device_removed", usn
        l_id = usn.split('::')[0]
        device = self.devices[l_id]
        self.device_removed(l_id)
        #print device.get_friendly_device_type()
        if device.get_friendly_device_type() == 'MediaServer':
            self.UPnP_ControlPoint_MediaServer_removed(l_id)
        if device.get_friendly_device_type() == 'MediaRenderer':
            self.UPnP_ControlPoint_MediaServer_removed(l_id)
        reactor.callLater(1, self.remove, l_id)
